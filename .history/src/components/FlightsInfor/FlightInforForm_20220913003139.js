...
function FlightInforForm({ img, name, setShowFlighDEtail, setShowFareInfo }) {
  const blueText = {
  ...
  return (
    <div className="FlightInforForm-container">
      ...
      <ul className="FlightInforForm-details">
        <li onClick={() => setShowFlighDEtail((oldState) => !oldState)}>
          Flight detail
        </li>
        <li onClick={() => setShowFareInfo((oldState) => !oldState)}>
          Fare info
        </li>
      </ul>
      <div className=""></div>
